...
export type TmdbPersonDetail = {
  ...
  movie_credits?: { cast: TmdbPersonMovieCredit[]; crew: TmdbPersonMovieCredit[] };
};
...
async function tmdb<T>(path: string, params: Record<string, string | number | undefined> = {}) {
  if (!env.TMDB_API_KEY) {
    ...
    return { page: 1, total_pages: 0, total_results: 0, results: [] } as unknown as T;
  }
  ...
  const res = await fetch(url, {
    headers: isBearer
      ? { Authorization: `Bearer ${env.TMDB_API_KEY}`, Accept: "application/json" }
...
export const tmdbApi = {
  ...
  trending(window: "day" | "week" = "day", page = 1) {
    return tmdb<TmdbPaged<TmdbMovieSummary>>(`/trending/movie/${window}`, { page });
  },
  person(id: number) {
