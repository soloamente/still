...
import { invalidateMovieDetailCache } from "../lib/listing-detail-cache";
import { syncMoviePosterPalette } from "../lib/sync-movie-palette";
import { type TmdbMovieSummary, tmdbApi, tmdbImg } from "../lib/tmdb";

const TMDB_VIRTUAL_SOURCES: {
	id: string;
	name: string;
	kind: "tmdb_trending" | "tmdb_upcoming" | "tmdb_now_playing" | "tmdb_popular";
	fetcher: () => Promise<{ results: TmdbMovieSummary[] }>;
}[] = [
	{
		id: "tmdb_trending",
		name: "Trending on TMDb",
		kind: "tmdb_trending",
		fetcher: () => tmdbApi.trending("day"),
	},
	{
		id: "tmdb_upcoming",
		name: "Upcoming Releases",
		kind: "tmdb_upcoming",
		fetcher: () => tmdbApi.upcoming(1),
	},
	{
		id: "tmdb_now_playing",
		name: "In Theaters Now",
		kind: "tmdb_now_playing",
		fetcher: () => tmdbApi.nowPlaying(1),
	},
	{
		id: "tmdb_popular",
		name: "Popular on TMDb",
		kind: "tmdb_popular",
		fetcher: () => tmdbApi.popular(1),
	},
];
			...
			for (const result of page.results) {
				...
				await db
					...
					.values({
						...
						imageUrl:
							tmdbImg.backdrop(result.backdrop_path) ??
							tmdbImg.poster(result.poster_path),
...
export async function refreshStaleMovies() {
	...
	for (const { id } of stale) {
		try {
			...
			void invalidateMovieDetailCache(id).catch(() => {});
		} catch (err) {
