...
async function snapshot(userId: string) {
  ...
  return {
    ...
    genreCounts: new Map((genres.rows ?? []).map((r) => [Number(r.gid), Number(r.c)])),
    decadeCounts: new Map((decades.rows ?? []).map((r) => [Number(r.decade), Number(r.c)])),
...
function meetsCriteria(c: Criteria, s: Snapshot): boolean {
  switch (c.kind) {
    ...
    case "decade_coverage": {
      ...
      for (const [, count] of s.decadeCounts) if (count >= required) coveredDecades += 1;
...
async function awardBadge(userId: string, badgeRow: typeof badge.$inferSelect, ctx: Record<string, unknown>) {
  await db
    .insert(userBadge)
    .values({
      userId,
  ...
  await db.insert(notification).values({
    ...
    payload: { badgeId: badgeRow.id, tier: badgeRow.tier, points: badgeRow.points },
  });
}
...
async function progressAchievement(
  ...
  if (met) {
    await db.insert(notification).values({
      ...
      body: a.description,
      payload: { achievementId: a.id, points: a.points },
    });
  }
}
    ...
    for (const b of badgeRows) {
      if (meetsCriteria(b.criteria as Criteria, s)) {
        const [existing] = await db
          ...
          .limit(1);
        if (!existing) await awardBadge(userId, b, { snapshot: { logs: s.logsCount } });
      }
    }
