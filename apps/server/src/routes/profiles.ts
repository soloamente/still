import { db, follow, list, log, movie, profile, review, user, userBadge } from "@still/db";
 ...
 */
export const profilesRoute = new Elysia({ prefix: "/api/profiles", tags: ["profiles"] })
  .use(context)
  ...
  .patch(
    ...
    async ({ body, user, status }) => {
      ...
      const [existing] = await db.select().from(profile).where(eq(profile.userId, user.id)).limit(1);

      // Handle availability check (case-insensitive).
      if (body.handle) {
        const desired = body.handle.toLowerCase();
        if (!HANDLE_RE.test(desired)) return status(400, "Invalid handle format");
        const [taken] = await db
          .select()
          .from(profile)
          .where(and(eq(profile.handle, desired), sql`${profile.userId} <> ${user.id}`))
          .limit(1);
      ...
      if (!body.handle || !body.displayName) {
        return status(400, "handle and displayName are required on first profile creation");
      }
      const [row] = await db
        .insert(profile)
        .values({
  ...
  .post("/me/banner", async ({ request, user, status }) => {
    ...
    try {
      ...
      if (msg.includes("private store")) {
        return status(502, {
          error: "Blob store is private but upload used public access (or the reverse).",
    ...
    await db.update(profile).set({ bannerUrl: blob.url }).where(eq(profile.userId, user.id));
  ...
  .get(
    ...
    async ({ params }) => {
      const handle = params.handle.toLowerCase();
      if (!HANDLE_RE.test(handle)) return { available: false, reason: "format" as const };
      const [taken] = await db
  ...
  .get("/:handle", async ({ params, user: viewer, status }) => {
    const handle = params.handle.toLowerCase();
    const [row] = await db
      .select({ user, profile })
      .from(profile)
      ...
      .where(eq(profile.handle, handle))
      .limit(1);
    if (!row || !row.user) return status(404, "Not found");
    ...
    if (viewer) {
      const [f] = await db
        .select()
        .from(follow)
        .where(and(eq(follow.followerId, viewer.id), eq(follow.followingId, row.user.id)))
        .limit(1);
    ...
    const pinned = await db
      .select()
      .from(userBadge)
      .where(and(eq(userBadge.userId, row.user.id), eq(userBadge.isPinned, true)))
      .limit(8);
