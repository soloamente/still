import { db, follow, list, log, movie, profile, review, user } from "@still/db";
    ...
    async ({ user: viewer, status, query }) => {
      ...
      const items = [
        ...
        ...reviews
          ...
          .map((row) => ({ kind: "review" as const, at: row.review.publishedAt, payload: row })),
        ...lists.map((row) => ({
          kind: "list" as const,
