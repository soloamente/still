import { db, list, listItem, movie, reaction, eventLog } from "@still/db";
...
export const listsRoute = new Elysia({ prefix: "/api/lists", tags: ["lists"] })
  ...
  .get(
    "/:id",
    async ({ params, status }) => {
      const [meta] = await db.select().from(list).where(eq(list.id, params.id)).limit(1);
  ...
  .patch(
    ...
    async ({ params, body, user, status }) => {
      if (!user) return status(401, "Sign in");
      const [existing] = await db.select().from(list).where(eq(list.id, params.id)).limit(1);
  ...
  .post(
    ...
    async ({ params, body, user, status }) => {
      if (!user) return status(401, "Sign in");
      const [parent] = await db.select().from(list).where(eq(list.id, params.id)).limit(1);
  ...
  .delete(
    ...
    async ({ params, user, status }) => {
      ...
      await db
        .delete(listItem)
        .where(
          and(eq(listItem.listId, params.id), eq(listItem.movieId, Number(params.movieId))),
        );
      await db
        .update(list)
        .set({ itemsCount: sql`(select count(*) from list_item where list_id = ${params.id})` })
        .where(eq(list.id, params.id));
      return { ok: true };
    },
