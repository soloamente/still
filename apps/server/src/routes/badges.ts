...
export const badgesRoute = new Elysia({ prefix: "/api/badges", tags: ["gamification"] })
  .use(context)
  .get("/catalog", async () => {
    const rows = await db.select().from(badge).orderBy(badge.category, badge.tier);
    return rows;
  })
  ...
  .get(
    ...
    async ({ user, status, query }) => {
      if (!user) return status(401, "Sign in");
      const since = query.since ? new Date(query.since) : new Date(Date.now() - 5 * 60_000);
      const rows = await db
        .select({ userBadge, badge })
        .from(userBadge)
        .leftJoin(badge, eq(userBadge.badgeId, badge.id))
        .where(
          and(eq(userBadge.userId, user.id), sql`${userBadge.awardedAt} > ${since}`),
        )
        .orderBy(desc(userBadge.awardedAt));
      return rows;
    },
  ...
  .post(
    ...
    async ({ params, user, status, body }) => {
      ...
      await db
        ...
        .where(and(eq(userBadge.userId, user.id), eq(userBadge.badgeId, params.badgeId)));
      return { ok: true };
    },
    {
...
export const achievementsRoute = new Elysia({
  ...
  .use(context)
  .get("/catalog", async () => {
    const rows = await db.select().from(achievement).orderBy(achievement.points);
    return rows;
  })
  .get("/me", async ({ user, status }) => {
    if (!user) return status(401, "Sign in");
