...
export const postsRoute = new Elysia({ prefix: "/api/posts", tags: ["posts"] })
  ...
  .post(
    ...
    },
    {
      body: t.Object({
        ...
        kind: t.Optional(t.Union([t.Literal("status"), t.Literal("share"), t.Literal("milestone")])),
  ...
  .delete("/:id", async ({ params, user: viewer, status }) => {
    if (!viewer) return status(401, "Sign in");
    const [existing] = await db.select().from(post).where(eq(post.id, params.id)).limit(1);
