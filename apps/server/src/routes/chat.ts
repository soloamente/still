...
async function threadsForUser(userId: string) {
  const memberships = await db
    .select({ threadId: chatMember.threadId, lastReadAt: chatMember.lastReadAt })
    .from(chatMember)
...
export const chatRoute = new Elysia({ prefix: "/api/chat", tags: ["chat"] })
  ...
  .post(
    ...
    async ({ body, user: viewer, status }) => {
      ...
      if (memberIds.length < 2) return status(400, "Need at least one other member");
  ...
  .get(
    ...
    async ({ params, user: viewer, query, status }) => {
      ...
      const [member] = await db
        .select()
        .from(chatMember)
        .where(and(eq(chatMember.threadId, params.id), eq(chatMember.userId, viewer.id)))
        .limit(1);
    ...
    async ({ params, body, user: viewer, status }) => {
      ...
      for (const m of members) {
        ...
        await db.insert(notification).values({
          ...
          body: preview,
          payload: { threadId: params.id, messageId: id },
        });
      }
