...
export const followsRoute = new Elysia({ prefix: "/api/follows", tags: ["follows"] })
  .use(context)
  .post(
    "/:userId",
    async ({ params, user: viewer, status }) => {
      if (!viewer) return status(401, "Sign in");
      if (viewer.id === params.userId) return status(400, "Cannot follow yourself");
      if (!hit(`follow:${viewer.id}`, { limit: 60, windowMs: 60_000 }).ok)
        return status(429, "Slow down");

      const [target] = await db.select().from(user).where(eq(user.id, params.userId)).limit(1);
      ...
      const [reciprocal] = await db
        .select()
        .from(follow)
        .where(and(eq(follow.followerId, params.userId), eq(follow.followingId, viewer.id)))
        .limit(1);
      if (reciprocal) {
        await db
          .update(follow)
          .set({ isMutual: true })
          .where(and(eq(follow.followerId, viewer.id), eq(follow.followingId, params.userId)));
        await db
          .update(follow)
          .set({ isMutual: true })
          .where(and(eq(follow.followerId, params.userId), eq(follow.followingId, viewer.id)));
