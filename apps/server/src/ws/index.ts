...
export const wsRoute = new Elysia({ tags: ["ws"] }).ws("/ws/chat", {
  body: t.Object({
    kind: t.Union([t.Literal("join"), t.Literal("leave"), t.Literal("typing"), t.Literal("ping")]),
  ...
  open(ws) {
    const userId = (ws.data.request as unknown as { __userId?: string }).__userId;
    if (!userId) {
      ws.close();
      return;
    }
    (ws.data as unknown as { store: { userId: string; joined: Map<string, () => void> } }).store =
      { userId, joined: new Map<string, () => void>() };
  },
  async message(ws, msg) {
    const store = (ws.data as unknown as {
    ...
    if (msg.kind === "join" && msg.threadId) {
      const [member] = await db
        .select()
        .from(chatMember)
        .where(and(eq(chatMember.threadId, msg.threadId), eq(chatMember.userId, store.userId)))
        .limit(1);
      if (!member) {
      ...
      const unsubscribe = subscribe(msg.threadId, (payload) => ws.send(payload));
