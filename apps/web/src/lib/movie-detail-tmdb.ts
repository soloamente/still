...
const FESTIVAL_AWARD_RE =
	/festival|oscar|cannes|berlinale|venice film|venice\b|sundance|tribeca|bafta|golden globe|palme d|award|nomination|sxsw|rotterdam|tiff\b|toronto international|bfi london|locarno|karlovy|telluride|busan|biff\b|beyond fest|beyondfest|zurich|zff\b|mill valley|mvff\b|independent spirit|mtv/i;
...
export type PremiereRow = {
	region: string;
	date: string;
	note?: string;
	kind: "Premiere" | "Limited";
};
...
export function extractPremiereRows(
	...
	deduped.sort(
		(a, b) => a.date.localeCompare(b.date) || a.region.localeCompare(b.region),
	);
	return deduped;
}
...
export function festivalAndAwardKeywordNames(
	...
	const hits = keywords
		.filter((k) => FESTIVAL_AWARD_RE.test(k.name))
		.map((k) => k.name);
	return [...new Set(hits)].sort((a, b) => a.localeCompare(b));
}

/**
 * TMDb TV recommendation/similar rows use `name` + `poster_path`; normalize so
 * `mergeMoreLikeThis` and poster grids can reuse the film summary shape (`title`).
 */
export function tvNameRowsToMovieSummaries(
	rows:
		| {
				id: number;
				name?: string;
	...
	return rows.map((r) => ({
		id: r.id,
		title: (r.name ?? r.title ?? "").trim() || "Untitled",
		poster_path: r.poster_path ?? null,
	}));
}

/** TV append `keywords` uses `results[]`; movies use nested `keywords[]`. */
export function tvDetailKeywordList(
	keywords:
		| {
	...
	if (!keywords) return [];
	if (keywords.results?.length) return keywords.results;
	if (keywords.keywords?.length) return keywords.keywords;
	return [];
}
