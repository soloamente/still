...
import { Calendar, Clock, Quote, Star, Users } from "lucide-react";
...
export async function generateMetadata({ params }: { params: Promise<Params> }) {
  ...
  const res = await api.api.movies({ id })?.get?.().catch(() => ({ data: null }));
...
type TmdbJsonShape = {
  ...
  similar?: { results?: { id: number; title: string; poster_path: string | null }[] };
  recommendations?: { results?: { id: number; title: string; poster_path: string | null }[] };
  ...
  "watch/providers"?: {
    results?: Record<
      string,
      { link?: string; flatrate?: { provider_name: string; logo_path: string }[] }
    >;
  };
} | null;
...
export default async function MoviePage({ params }: { params: Promise<Params> }) {
  ...
  const reviewsRes = await api.api.movies({ id }).reviews.get().catch(() => ({ data: [] }));
  const reviews = (reviewsRes.data as unknown as ReviewRow[]) ?? [];

  const j = data.tmdbJson;
  const directors = j?.credits?.crew?.filter((c) => c.job === "Director").slice(0, 2) ?? [];
  const topCast = j?.credits?.cast?.slice(0, 14) ?? [];
  const crewRows = buildCrewRows(j?.credits?.crew as Parameters<typeof buildCrewRows>[0]);
  /** The table stays terse; the crawl can surface more departmental roles behind the marquee. */
  const crewRowsForCrawl = buildCrewRows(j?.credits?.crew as Parameters<typeof buildCrewRows>[0], 80);
  ...
  const providersUs = j?.["watch/providers"]?.results?.US?.flatrate?.slice(0, 6) ?? [];
  ...
  const recognitionPresent = premiereRows.length > 0 || festivalKeywords.length > 0;

  const { accent: movieAccent } = accentFromGenres(j?.genres);
  const doubleFeaturePick = moreLikeThis.find((m) => m.id !== data.tmdbId) ?? null;
  ...
  const featuredReviews = [...reviews]
    ...
    .sort((a, b) => b.likesCount - a.likesCount || b.body.length - a.body.length)
    .slice(0, 2);
  ...
  return (
    ...
    >
      ...
      <section className="cinema-hero-iris movie-hero-glow relative left-1/2 w-screen max-w-[100vw] -translate-x-1/2 overflow-hidden">
        ...
        <Letterbox aspect="2.39" bars={false} className="pointer-events-none relative z-0 w-full">
          {/*
        ...
        <div className="relative z-20 isolate pointer-events-auto w-full px-3 pb-12 pt-0 sm:px-4 md:mx-auto md:-mt-24 md:max-w-7xl md:px-5 md:pb-20 md:pt-5">
          {/*
          ...
          <div className="flex flex-col items-center gap-8">
            ...
            <div className="w-full max-w-3xl space-y-4 px-3 text-center sm:px-4 md:px-0">
              <p className="text-xs uppercase tracking-wider text-muted-foreground">
                {data.releaseDate ? formatDate(new Date(data.releaseDate)) : data.year}
              </p>
              <h1 className="font-display text-4xl font-medium leading-[1] tracking-[-0.02em] md:text-6xl">
                {data.title}
              </h1>
              {data.tagline ? (
              ...
              <ul className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
                ...
                {j?.genres?.slice(0, 3).map((g) => (
                  <li key={g.id} className="rounded-md border border-border px-1.5 py-0.5">
                    {g.name}
                  </li>
                ))}
              </ul>
      ...
      <div className="mx-auto max-w-7xl space-y-12 px-3 py-12 sm:px-4 md:px-5">
        {data.overview ? (
          <Section title="Synopsis">
            <p className="font-editorial max-w-3xl text-lg text-foreground/85">{data.overview}</p>
          </Section>
        ) : null}

        {topCast.length || crewRows.length ? (
          <Section title="Cast & crew" subtitle="Headliners and key creative credits from TMDb.">
            {topCast.length ? (
              <div className="mb-8">
                ...
                <ul className="flex gap-3 overflow-x-auto pb-2 [-webkit-overflow-scrolling:touch]">
                  {topCast.map((c) => (
                    <li key={`${c.id}-${c.character ?? ""}`} className="w-24 shrink-0 text-center">
                      <Link
                        href={`/people/${c.id}`}
              ...
              <div className="mb-6">
                ...
                <ul className="space-y-1.5 text-sm">
                  {premiereRows.slice(0, 12).map((r) => (
                    <li key={`${r.region}-${r.date}-${r.kind}`} className="flex flex-wrap gap-x-2 gap-y-0.5">
        ...
        {providersUs.length ? (
          <Section title="Where to watch">
            <p className="mb-2 text-xs text-muted-foreground">Streaming in the US</p>
              ...
              {tmdbAvg != null && tmdbVotes > 0 ? (
                <>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {tmdbVotes.toLocaleString()} logged votes on The Movie Database
                  </p>
                </>
              ) : (
                <p className="mt-3 text-sm text-muted-foreground">No aggregate score yet.</p>
              )}
            </div>
            <div className="rounded-2xl border border-border bg-card/50 p-5">
              <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">
              ...
              {data.community.averageRating != null && data.community.reviewsCount > 0 ? (
                <>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Averaged from {data.community.reviewsCount} published reviews
                  </p>
                </>
              ) : (
        ...
        {featuredReviews.length ? (
          <Section
            title="Critics & member reviews"
            subtitle="We do not license professional critics yet — here are standout write-ups from the community."
          >
            <ul className="mb-8 grid gap-4 md:grid-cols-2">
              {featuredReviews.map((r) => (
                ...
                >
                  <Quote
                    className="absolute left-3 top-3 size-5 text-desert-orange/50"
                    aria-hidden
                  />
                  <Link href={`/reviews/${r.id}`} className="group block">
                    {r.title ? (
                      <p className="font-serif text-lg group-hover:text-desert-orange">{r.title}</p>
                    ) : null}
                    <p className="font-editorial mt-2 line-clamp-5 text-sm text-foreground/85">
                      {r.body}
                    </p>
                    <div className="mt-3 flex flex-wrap items-center gap-x-2 text-xs text-muted-foreground">
                      <span>
                        Still member · {r.likesCount} likes
                      </span>
            ...
            {reviewsAfterFeatured.length ? (
              <>
                ...
                <ul className="grid gap-4 md:grid-cols-2">
                  {reviewsAfterFeatured.slice(0, 8).map((r) => (
                    <li key={r.id}>
                      <ReviewCard review={r} />
                    </li>
                  ))}
        ...
        {moreLikeThis.length ? (
          <Section
            kicker="Repertory carousel"
            title="More films like this"
            subtitle="Blended from TMDb recommendations and related titles."
          >
        ...
        {doubleFeaturePick ? (
          <DoubleFeatureSuggestion currentTitle={data.title} pick={doubleFeaturePick} />
        ) : null}
        ...
        {crewCrawlLines.length ? (
          ...
          >
            ...
            <CreditsCrawl
              ...
              durationSec={Math.min(420, Math.max(160, crewCrawlLines.length * 22))}
            />
          </Section>
        ) : null}
