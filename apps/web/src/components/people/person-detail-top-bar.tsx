...
const PERSON_DETAIL_VIEW_TABS: readonly {
	id: PersonDetailView;
	label: string;
}[] = [
...
/**
 * Person detail header — smart back pill, About · Filmography tabs on the raised
 * `bg-card` track with a sliding pill, and share (matches film detail chrome).
 */
export function PersonDetailTopBar({
	personId,
	title,
	view,
	detailBasePath,
	...
	onViewChange?: (view: PersonDetailView) => void;
}) {
	const basePath = detailBasePath ?? `/people/${personId}`;
	...
	function buildViewHref(nextView: PersonDetailView) {
		return buildPersonDetailViewHref(basePath, nextView);
	}
	...
	function handleViewSelect(nextView: PersonDetailView) {
		...
		if (lobbyNav) {
			return (
				...
				>
					{active ? (
						<motion.span
							layoutId="person-detail-view-pill"
	...
	return (
		...
		>
			<div className="grid w-full grid-cols-[1fr_auto_1fr] items-center gap-2 px-2.5 py-2 sm:px-3">
				...
				>
					{PERSON_DETAIL_VIEW_TABS.map((tab) => (
