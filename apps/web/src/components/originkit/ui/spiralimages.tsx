...
// Framer RenderTarget stub — Originkit export; keep so canvas vs preview branches stay valid.
const RenderTarget = {
	current: () => "preview",
	canvas: "canvas",
	export: "export",
...
export default function SpiralImages(props: any) {
	...
	useEffect(() => {
		imgsRef.current = items.map((im: any) => {
			...
			const el = new Image();
			/*
			  Display-only canvas draw — do NOT set crossOrigin.
			  `anonymous` + a CDN that omits ACAO (or strips it under privacy
			  modes) makes the image fail entirely, so the spiral only paints
			  colored placeholders. We never call getImageData/toDataURL here.
			*/
			el.decoding = "async";
			el.src = im.src;
			return el;
		...
		const draw = (now: number) => {
			...
			for (let k = 0; k < cards.length; k++) {
				...
				if (ready) {
					...
					// Quiet loading tile — not a rainbow placeholder that reads as broken.
