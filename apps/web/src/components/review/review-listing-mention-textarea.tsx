...
type ReviewListingMentionTextareaProps = {
	id: string;
	value: string;
...
/**
 * Review body field with `@` autocomplete for films and TV shows.
 * Inserts `@[Title](/movies|tv/id)` tokens readers can tap later.
 */
export function ReviewListingMentionTextarea({
	id,
	value,
	onChange,
	className,
	...
	maxLength,
	placeholder,
}: ReviewListingMentionTextareaProps) {
	...
	const handleSelect = (index: number) => {
		...
		const { nextBody, nextCursor } = insertReviewListingMention(value, range, {
			title: hit.title,
				...
				{loading ? (
					...
					<p className="px-3 py-4 text-center text-muted-foreground text-sm">
						{mentionQuery
							...
							: "Type a film or TV title to tag it"}
					</p>
				) : (
