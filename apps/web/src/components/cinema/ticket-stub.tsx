...
export function TicketStub({
	...
	const compactPosterFillFlexible =
		size === "compact" && posterFillTicket && posterFillFlexible;

	/** Grid Popular: Ø38 punches; Coming-soon / other compact rails: preset (smaller notch). */
	const posterFillNotchClass = compactPosterFillFlexible
		? "size-[38px]"
		: preset.notch;
	...
	const outerArticle = cn(
		...
		posterFillTicket
			? cn(
					posterFillFlexible
						? "mx-0 w-full min-w-0 shrink"
						: cn("mx-auto shrink-0", preset.fullBleedWidth),
					"max-w-none",
				)
	...
	const posterFillRailBody = posterFillTicket ? (
		<>
					...
					<Image
						...
						className={cn(
							...
							(size === "default" || posterFillTicket) &&
								cn(
									"transition-[filter] duration-200 ease-out [@media(hover:hover)]:group-hover:brightness-[1.04]",
			...
			<div
				aria-hidden
				className={cn(
					"pointer-events-none absolute inset-x-0 bottom-0 z-[11] bg-gradient-to-t from-black/82 via-black/44 to-transparent transition-opacity duration-200 ease-out will-change-auto",
					size === "compact"
						? compactPosterFillFlexible
							? "h-[7rem] sm:h-[7.25rem]"
							: "h-[5.25rem]"
			...
			<div
				className={cn(
					...
					compactPosterFillFlexible
						? "inset-x-0 bottom-3 px-3 pt-8 pb-2 text-[0.9375rem] leading-snug sm:text-[clamp(0.9375rem,2.05vw,1.0625rem)] md:text-[1rem] md:leading-normal lg:text-[1.0625rem] xl:text-[1.125rem]"
						: "inset-x-0 bottom-[10px] px-2 pt-5 pb-0.5 text-[0.7rem] leading-snug",
					/*
			...
			>
				{stubKicker ? (
					<p
						className={cn(
							"font-medium text-white/92 uppercase",
							compactPosterFillFlexible
				...
				{/* Title row — callers should keep `font-sans` parity (home rail drops Fraunces for UI sans) */}
				<div>{children}</div>
			</div>
	...
	const article = posterFillTicket ? (
		...
		<article className={outerArticle}>
			<div
				className={cn(
					"relative aspect-[2/3] w-full overflow-hidden bg-black/40",
					preset.roundedTop,
				)}
			>
				{resolved ? (
			...
			>
				...
				<div className="border-white/35 border-t border-dashed pt-3">
					{stubKicker ? (
						<p className="mb-3 font-display font-medium text-[9px] text-white/72 uppercase tracking-[0.32em] md:text-[8px]">
							{stubKicker}
						</p>
					) : null}
					<div>{children}</div>
	...
	if (href) {
		return (
			<Link
				...
				className={cn(
					"cinema-ticket-link group block outline-none transition-transform duration-200 ease-out will-change-auto focus-visible:ring-2 focus-visible:ring-[color:var(--color-desert-orange)] focus-visible:ring-offset-2 focus-visible:ring-offset-background",
					posterFillTicket && posterFillFlexible
						? "mx-0 w-full"
						: "mx-auto w-fit",
					linkHoverGrow &&
