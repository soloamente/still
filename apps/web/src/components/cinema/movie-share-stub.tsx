	...
	return (
		<div className="rounded-2xl border border-border bg-card/45 p-5">
			<div className="flex flex-wrap items-end justify-between gap-4">
				<div>
					<p className="font-semibold text-[10px] text-muted-foreground uppercase tracking-[0.28em]">
						Admit one
					</p>
					<p className="mt-2 font-display text-lg tracking-[-0.02em]">
						{title}
					</p>
					<p className="mt-1 text-muted-foreground text-xs">
						Scan-style stub is decorative — copy the real link for your group
						chat.
					</p>
				</div>
				<Button
			...
			<p className="mt-2 text-[10px] text-muted-foreground uppercase tabular-nums tracking-[0.3em]">
				{APP_NAME} · {movieId.toString().padStart(7, "0")}
			</p>
		</div>
	);
}
