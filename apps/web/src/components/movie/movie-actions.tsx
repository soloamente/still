...
export function MovieActions({ movieId, title }: { movieId: number; title: string }) {
  ...
  const [busy, setBusy] = useState<null | "log" | "watchlist" | "like">(null);
  ...
  const refreshUserState = useCallback(async () => {
    ...
    if (!wlRes.error && wlPayload) setInWatchlist(Boolean(wlPayload.inWatchlist));
  ...
  async function quickLog() {
    ...
    try {
      ...
      if (!result.ok || !result.data) {
        console.error("[movie-actions] log post failed", result.error);
        toast.error("Couldn't log this film");
        return;
      }
      void play("reel-clack").catch(() => undefined);
      toast.success(myLogs.length ? `Logged “${title}” again` : `Logged “${title}”`);
      await refreshUserState();
    } catch (err) {
      console.error(err);
      toast.error("Couldn't log this film");
  ...
  return (
    <div className="relative z-[31] isolate flex flex-wrap items-center gap-2 pt-2">
      <Button variant="accent" size="pill" onClick={quickLog} disabled={!hydrated || busy === "log"}>
        {!hydrated ? (
      ...
      >
        {busy === "like" ? (
          ...
          <Heart className={`size-3.5 ${likedVisual ? "fill-current" : ""}`} aria-hidden />
        )}
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={() => toast.info("List picker coming soon")}>
            <Plus className="mr-2 size-3.5" aria-hidden /> New list with this film
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
