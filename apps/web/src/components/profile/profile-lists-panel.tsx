...
import { Film, ListMusic, Lock } from "lucide-react";
...
/** Narrow left rail — same `bg-background` as the card, no tint or divider. */
...
function ProfileListTile({ list }: { list: ListBoardRow }) {
	return (
		<Link
			href={`/lists/${list.id}`}
			className={cn(
...
/**
 * Patron lists tab — narrow full-height cover stack on the left, copy on the right.
 */
export function ProfileListsPanel({ lists }: { lists: ListBoardRow[] }) {
	if (!lists.length) {
		return (
			<ProfileSocialEmpty
				icon={ListMusic}
