...
**Implementation in this repo:** CSS custom properties live in `packages/ui/src/styles/globals.css` under the `--mobbin-*` prefix (Tailwind utilities `*-mobbin-*`, e.g. `bg-mobbin-mist`, `text-mobbin-midnight-ink`) so they never override Aker tokens such as `--color-fog`. The body UI stack (`--font-inter` / Outfit) substitutes for saans where the commercial font is unavailable.

## Tokens — Colors

| Name | Value | Token | Role |
|------|-------|-------|------|
| Midnight Ink | `#141414` | `--mobbin-midnight-ink` | Primary text, headings, filled CTA buttons, nav items, icon strokes — the single chromatic workhorse of an achromatic system |
| Pure Canvas | `#ffffff` | `--mobbin-pure-canvas` | Page background, card surfaces, button text on dark fills |
| Graphite | `#707070` | `--mobbin-graphite` | Body copy, secondary links, descriptive text |
| Ash | `#adadad` | `--mobbin-ash` | Tertiary text, disabled/muted button borders, placeholder icons |
| Fog | `#ededed` | `--mobbin-fog` | Dividers, subtle borders, card outlines |
| Mist | `#f2f2f2` | `--mobbin-mist` | Nav background tint, input fields, inner surface elevation |
| Silver | `#c2c2c2` | `--mobbin-silver` | Skeleton loaders, inactive UI fills |
| Slate Shadow | `#e0e0e0` | `--mobbin-slate-shadow` | Inset button shadow ring (rgba(64,64,64,0.16) 0px 0px 0px 1px) |

## Tokens — Typography

### saans — Every text element on the site. Fractional weights (440, 456, 652) are the signature — they occupy visual mass between Regular and Semibold that no variable-axis font exposes as named stops. Display headlines at 80px/56px run weight 600–652 at lineHeight 1.00–1.13 with negative letter-spacing (-0.011em at large sizes). Body runs 14–16px at weight 400 with tracking +0.013–0.017em for small-size legibility. · `--mobbin-font-saans`

- **Substitute:** Inter Variable or Geist (closest variable-axis fallbacks with comparable optical geometry). **Still:** Outfit via `--font-inter` until a dedicated saans file is licensed.
...
|------|------|-------------|----------------|-------|
| caption | 12px | 16 | 0.2px | `--mobbin-text-caption` |
| body-sm | 14px | 20 | — | `--mobbin-text-body-sm` |
| body | 16px | 22 | 0.21px | `--mobbin-text-body` |
| subheading | 20px | 28 | 0.28px | `--mobbin-text-subheading` |
| heading | 32px | 42 | — | `--mobbin-text-heading` |
| heading-lg | 56px | 63 | -0.39px | `--mobbin-text-heading-lg` |
| display | 80px | 80 | -0.88px | `--mobbin-text-display` |
...
| Name | Value | Token |
|------|-------|-------|
| 8 | 8px | `--mobbin-spacing-8` |
| 16 | 16px | `--mobbin-spacing-16` |
| 24 | 24px | `--mobbin-spacing-24` |
| 32 | 32px | `--mobbin-spacing-32` |
| 40 | 40px | `--mobbin-spacing-40` |
| 64 | 64px | `--mobbin-spacing-64` |
| 80 | 80px | `--mobbin-spacing-80` |
| 104 | 104px | `--mobbin-spacing-104` |
| 120 | 120px | `--mobbin-spacing-120` |
...
| Name | Value | Token |
|------|-------|-------|
| subtle | `rgba(64, 64, 64, 0.16) 0px 0px 0px 1px inset` | `--mobbin-shadow-subtle` |
| xl | `rgba(0, 0, 0, 0.04) 0px 8px 40px 0px` | `--mobbin-shadow-xl` |
...
### CSS custom properties (repo: use `--mobbin-*` prefix in `globals.css`)

```css
:root {
  --mobbin-midnight-ink: #141414;
  --mobbin-pure-canvas: #ffffff;
  --mobbin-graphite: #707070;
  --mobbin-ash: #adadad;
  --mobbin-fog: #ededed;
  --mobbin-mist: #f2f2f2;
  --mobbin-silver: #c2c2c2;
  --mobbin-slate-shadow: #e0e0e0;
  --mobbin-font-saans: var(--font-inter), ui-sans-serif, system-ui, sans-serif;
  --mobbin-text-caption: 12px;
  --mobbin-text-body-sm: 14px;
  --mobbin-text-body: 16px;
  --mobbin-text-subheading: 20px;
  --mobbin-text-heading: 32px;
  --mobbin-text-heading-lg: 56px;
  --mobbin-text-display: 80px;
  --mobbin-tracking-caption: 0.2px;
  --mobbin-tracking-body: 0.21px;
  --mobbin-tracking-subheading: 0.28px;
  --mobbin-tracking-heading-lg: -0.39px;
  --mobbin-tracking-display: -0.88px;
  --mobbin-font-weight-regular: 400;
  --mobbin-font-weight-w440: 440;
  --mobbin-font-weight-w456: 456;
  --mobbin-font-weight-semibold: 600;
  --mobbin-font-weight-w652: 652;
  --mobbin-spacing-unit: 8px;
  --mobbin-spacing-8: 8px;
  --mobbin-spacing-16: 16px;
  --mobbin-spacing-24: 24px;
  --mobbin-spacing-32: 32px;
  --mobbin-spacing-40: 40px;
  --mobbin-spacing-64: 64px;
  --mobbin-spacing-80: 80px;
  --mobbin-spacing-104: 104px;
  --mobbin-spacing-120: 120px;
  --mobbin-page-max-width: 1280px;
  --mobbin-section-gap: 80px;
  --mobbin-radius-lg: 8px;
  --mobbin-radius-2xl: 16px;
  --mobbin-radius-2xl-2: 20px;
  --mobbin-radius-3xl: 24px;
  --mobbin-radius-full: 9999px;
  --mobbin-shadow-subtle: rgba(64, 64, 64, 0.16) 0px 0px 0px 1px inset;
  --mobbin-shadow-xl: rgba(0, 0, 0, 0.04) 0px 8px 40px 0px;
  --mobbin-surface-page: #ffffff;
  --mobbin-surface-card: #ffffff;
  --mobbin-surface-input-chip: #f2f2f2;
  --mobbin-surface-overlay-dropdown: #ffffff;
}
```

### Tailwind v4 (`@theme inline` in `globals.css`)

Maps the same values to utilities: `bg-mobbin-mist`, `text-mobbin-midnight-ink`, `border-mobbin-fog`, `shadow-mobbin-xl`, `font-mobbin-saans`, `max-w-mobbin-page`, etc.
...
{
  "color": {
    "midnight-ink": { "$value": "#141414", "$type": "color" },
    "pure-canvas": { "$value": "#ffffff", "$type": "color" },
    "graphite": { "$value": "#707070", "$type": "color" },
    "ash": { "$value": "#adadad", "$type": "color" },
    "fog": { "$value": "#ededed", "$type": "color" },
    "mist": { "$value": "#f2f2f2", "$type": "color" },
    "silver": { "$value": "#c2c2c2", "$type": "color" },
    "slate-shadow": { "$value": "#e0e0e0", "$type": "color" }
  },
  "font": {
    "saans": { "$value": "saans", "$type": "fontFamily" }
  },
  "spacing": {
    ...
    "80": { "$value": "80px", "$type": "dimension" },
    "104": { "$value": "104px", "$type": "dimension" },
    "120": { "$value": "120px", "$type": "dimension" }
  },
  "radius": {
    "lg": { "$value": "8px", "$type": "dimension" },
    "2xl": { "$value": "16px", "$type": "dimension" },
    "2xl-2": { "$value": "20px", "$type": "dimension" },
    "3xl": { "$value": "24px", "$type": "dimension" },
    "full": { "$value": "9999px", "$type": "dimension" }
  },
  "shadow": {
    "subtle": {
      "$value": "rgba(64, 64, 64, 0.16) 0px 0px 0px 1px inset",
      "$type": "shadow"
    },
    "xl": {
      "$value": "rgba(0, 0, 0, 0.04) 0px 8px 40px 0px",
      "$type": "shadow"
    }
  },
  "surface": {
    ...
    "input-chip": { "$value": "#f2f2f2", "$type": "color" },
    "overlay-dropdown": { "$value": "#ffffff", "$type": "color" }
  }
}
```
